...
import core.utilities.Generation;
...
@Log4j2
public class FreshTab extends ScrollWithFields<FreshAccount>
{
    public static DefaultComboBoxModel<FreshAccount> ACCOUNTS_MODEL = new DefaultComboBoxModel<>();
    ...
    JPanel detailsPanel;
    public FreshTab(Component parentComponent, Core core)
    {
        super(core, parentComponent, "Fresh", "Fresh");
    }
    @Override
    ...
    @Override
    public List<FreshAccount> getData()
    {
        return getCore().getDatabase().getAccountProvisioner().getFresh();
    }
    
    @Override
    public void init()
    {
        for(FreshAccount account : getData())
        {
        ...
        registerField(displayNameLabel, displayNameField, true, new ComplexFunction<>(Account::getDisplayName, Account::setDisplayName));
        ...
        registerField(notesLabel, notesField, true, new ComplexFunction<>(Account::getNotes, null));
    }
    ...
    @Override
    public FreshAccount getBlankWithDefaultValues()
    {
        FreshAccount account = new FreshAccount();
        account.setGameTime(0);
        account.setCash(0);
        account.setBankPin(-1);
        account.setDisplayName("");
        account.setCreationDate(ZonedDateTime.now());
        account.setBirthday(Generation.generateBirthday());
        account.setStartTime(Generation.generateStartTime());
        String seedWord = Generation.generateSeedWord();
        account.setEmail(seedWord + "@" + PrivateSharedConfig.BOT_DOMAIN_NAME);
        account.setPassword(seedWord + Generation.generateSeedWord() + ThreadLocalRandom.current().nextInt(1, 99));
    ...
    @Override
    public void updateTable(FreshAccount saveObject)
    {
        ACCOUNTS_MODEL.addElement(saveObject);
    }
    
    @Override
    public void saveToDatabase(FreshAccount saveObject)
    {
        getCore().getDatabase().getAccountProvisioner().getFreshAccs().saveOrUpdate(saveObject);
    }
